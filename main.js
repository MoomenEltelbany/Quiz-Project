    ...
    if (myRequest.readyState === 4 && myRequest.status === 200) {
      ...
      submitBtn.onclick = () => {
        ...
        if (indexCount === questionObjectLength - 1) {
          ...
          } else if (+rightAnswers.innerHTML < +totalAnswers.innerHTML) {
          ...
          let rightAnswer = questionObject[indexCount]["right_answer"];
...
function showQuestions(obj) {
  ...
  // Starting shuffling the array so that we shuffle the answers every time we start the game
  let arr = [1, 2, 3, 4];
